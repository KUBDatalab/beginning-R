...
keypoints:
   ...
   - "Functions allows you to do exactly the same calculations on lots of different data."
   - "Loops allows you to apply the same function to at lot of data"
   - "Logical tests allow you to apply different calculations on data, depending on the data."
source: Rmd
---
...
The **`tidyverse`** package includes the package **`ggplot2`**. If you want to 
make advanced, publication ready plots, we recommend using that. Here we are going
to take a look at at few of the fundamental options for plotting, build into 
R instead.

If not still in the workspace, load the data we saved in the previous lesson. 
Begin by loading the tidyverse package:
...
**`Base R`** is the term we use when we are relying solely on the standard functions
in R, without importing libraries to add to the functionality.

**`Base R`** plot functions are usually the quickest way to get at glimpse of 
the data we work with, and produces very "clean" plots without fancy details.

Getting **`Base R`** plots to look nice or have colours however is not that simple
or quick. For those purposes we would recommend the **`ggplot2`** package.

### Scatterplots ###

Scatterplots are plots of individual points in two dimensions, where the position
of the point is determined by the values of two variables, one determining the 
position on the X-axis, the other on the Y-axis.
